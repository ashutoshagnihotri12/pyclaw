...
struct pdeParam
{
	...
	real* waveSpeeds;			// speed of (each? fastest per riemann?) waves	//
		...
		: cellsX(256), cellsY(256), ghostCells(2), numStates(1), numWaves(1), numCoeff(1), startX(0), endX(1), startY(0), endY(1)
	{
		...
		int interfaces = (cellsX-1)*(cellsY) + (cellsX)*(cellsY-1);
		...
		cudaError_t alloc4 = cudaMalloc((void**)&waveSpeeds, interfaces*sizeof(real)/64);	// some safe margin storage for the wave speeds

		real* zeros = (real*)calloc(interfaces/64, sizeof(real));
		cudaMemcpy(waveSpeeds, zeros, interfaces*sizeof(real)/64, cudaMemcpyHostToDevice);
		free(zeros);
	};
		...
		startX(startX), endX(endX), startY(startY), endY(endY)
	{
		...
		int horizontal_blocks = ((CELLSX + HORIZONTAL_BLOCKSIZEX-3-1)/(HORIZONTAL_BLOCKSIZEX-3)) * ((CELLSY + HORIZONTAL_BLOCKSIZEY-1)/(HORIZONTAL_BLOCKSIZEY));
		int vertical_blocks = ((CELLSX + VERTICAL_BLOCKSIZEX-1)/(VERTICAL_BLOCKSIZEX)) * ((CELLSY + VERTICAL_BLOCKSIZEY-3-1)/(VERTICAL_BLOCKSIZEY-3));

		int total_blocks = horizontal_blocks + vertical_blocks;

		real* zeros = (real*)calloc(total_blocks, sizeof(real));
		cudaMemcpy(waveSpeeds, zeros, total_blocks*sizeof(real), cudaMemcpyHostToDevice);
		free(zeros);
	};
	...
	inline __device__ real &getElement_waveSpeed(int row, int column, int waveNum)
	{
		return waveSpeeds[getIndex_waveSpeed(row, column, waveNum)];
	}
	inline __device__ void setElement_waveSpeed(int row, int column, int waveNum, real waveSpeed)
	{
		waveSpeeds[getIndex_waveSpeed(row, column, waveNum)] = waveSpeed;
	}
};
