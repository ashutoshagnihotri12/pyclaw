	...
	! ===================================================================
		...
		do n = 1,num_eqn
			do i = num_ghost+1,maxnx+num_ghost-1
				...
				ql(n,i) = (1/128)*(-5*q(n,-2+i)+60*q(n,-1+i)+90*q(n,i)-20*q(n,1+i)+3*q(n,2+i));
				qr(n,i) = (1/128)*(3*q(n,-2+i)-20*q(n,-1+i)+90*q(n,i)+60*q(n,1+i)-5*q(n,2+i));
	...
	! ===================================================================
		...
		do n = 1,num_eqn
			do i = num_ghost+1,maxnx+num_ghost-1
				...
				ql(n,i) = (7*(q(n,-3+i) - 10*q(n,-2+i) + 75*q(n,-1+i) + 100*q(n,i) - 25*q(n,1+i) + 6*q(n,2+i)) - 5*q(n,3+i))/1024;
				qr(n,i) = (-5*q(n,-3+i) + 7*(6*q(n,-2+i) - 25*q(n,-1+i) + 100*q(n,i) + 75*q(n,1+i) - 10*q(n,2+i) + q(n,3+i)))/1024;
