...
__global__ void fused_Riemann_limiter_horizontal_update_kernel(pdeParam param, real dt, Riemann Riemann_solver_h, Limiter phi)
{
	...
	if (row < param.cellsY && col < param.cellsX-1)	// if there are 512 cells in X 511 threads are in use
	{
		...
		Riemann_solver_h(leftCell,			// input comes from global memory
							...
							leftCoeff,		//
							rightCoeff,		//
			/*shared*/		&getSharedWave(waves, threadIdx.y, threadIdx.x, 0, 0, numStates, numWaves, blockDim.x),
			/*shared*/		&getWaveSpeed(waveSpeeds, threadIdx.y, threadIdx.x, 0, numWaves, blockDim.x));
	}
	...
	// after which the pth component of fluctuation is stored in shared, this proved to be not so good
	...
	{
		if (tid < 96)
			waveSpeeds[tid] = fmax(waveSpeeds[tid], waveSpeeds[tid + 96]);
		__syncthreads();
	}
	if (blockSize >= 96)
	{
		if (tid < 64)
			waveSpeeds[tid] = fmax(waveSpeeds[tid], waveSpeeds[tid + 64]);
		__syncthreads();
	}
	if (tid < 32)
	...
	if (tid == 0)
		param.waveSpeeds[blockIdx.x + blockIdx.y*gridDim.x] =  waveSpeeds[0];
}

extern __shared__ real shared_elem[];
template <const int numStates, const int numWaves, const int numCoeff, const unsigned int blockSize, class Riemann ,class Limiter>
__global__ void fused_Riemann_limiter_vertical_update_kernel(pdeParam param, real dt, Riemann Riemann_solver_v, Limiter phi, int global_speeds_offset)
{
	...
	waveSpeeds[tid] = fmax(waveSpeeds[tid], -waveSpeeds[tid]);
	#pragma unroll
	for (int i = 1; i < numWaves; i++)
	{
		// blockDim.x * blockDim.y is usally a multiple of 32, so there should be no bank conflicts.
	...
	// Reducing over block size elements
	// use knowledge about your own block size:
	// I am using blocks of size 384 for the vertical direction
	if (blockSize >= 384)
	{
		if (tid < 192)
			waveSpeeds[tid] = fmax(waveSpeeds[tid], waveSpeeds[tid + 192]);
		__syncthreads();
	}
	if (blockSize >= 192)
	{
	...
	if (tid == 0)
		param.waveSpeeds[blockIdx.x + blockIdx.y*gridDim.x + global_speeds_offset] =  waveSpeeds[0];
	////?	param.waveSpeeds[blockIdx.x + blockIdx.y*gridDim.x + gridDim.x*gridDim.y + global_speeds_offset] =  waveSpeeds[0];
	//////////////////////////////////////////////////////////// what is thsi ^ ?    ///////////////////////////////////////
}
										...
										)
{
	...
	int global_speeds_offset;
	{
		...
		fused_Riemann_limiter_horizontal_update_kernel<3, 2, 2, blockDim_X*blockDim_Y, Riemann_h, Limiter><<<dimGrid_h, dimBlock_h, SharedMemorySize>>>(param, dt, Riemann_pointwise_solver_h, limiter_phi);

		global_speeds_offset = gridDim_X * gridDim_Y;
	}
	{
		...
		fused_Riemann_limiter_vertical_update_kernel<3, 2, 2, blockDim_X*blockDim_Y, Riemann_v, Limiter><<<dimGrid_v, dimBlock_v, SharedMemorySize>>>(param, dt, Riemann_pointwise_solver_v, limiter_phi, global_speeds_offset);

		global_speeds_offset += gridDim_X * gridDim_Y;
	}
	{
		const unsigned int blockDim_X = 256;		// fine tune the best block size
		
		size_t SharedMemorySize = (blockDim_X)*sizeof(real);
		global_speeds_offset = 16384;
		unsigned int gridDim_X = (global_speeds_offset + (blockDim_X-1)) / (blockDim_X + 4*1024);
		if (gridDim_X == 0)
			gridDim_X = 1;

		dim3 dimGrid(gridDim_X);
		dim3 dimBlock(blockDim_X);

		reduceMax<blockDim_X><<< dimGrid, dimBlock, SharedMemorySize>>>(param.waveSpeeds, global_speeds_offset);
	}
